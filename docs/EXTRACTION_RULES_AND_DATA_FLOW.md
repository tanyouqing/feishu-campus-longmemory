...
| LLM 工作偏好候选 | 飞书消息、OpenClaw message 事件 | 可选启用 | WorkPreferenceMemory |
| LLM 提醒候选 | 飞书消息、OpenClaw message 事件 | 可选启用 | ReminderPreferenceMemory，可解析时间时生成 reminder_jobs |
| LLM 用户画像 patch | 飞书消息、OpenClaw message 事件 | 可选启用 | user_profiles + user_profile_evidence_links |
...
当前显式抽取器只从 `WorkEvent.content_json.summary.text` 读取文本。1.0.1 起可选启用 LLM 记忆候选抽取层，1.1.0 起可选启用 LLM 用户画像抽取层。LLM 只返回候选结构，不直接写库；候选必须通过后端校验后再进入现有 Store。
...
2. 取消提醒：优先处理。
3. 忘记记忆：其次处理。
4. 如果启用 LLM 抽取，先请求 LLM 生成工作偏好或提醒候选。
5. 后端校验 LLM 候选，通过后调用现有 `MemoryStore.write_memory`。
6. 如果 LLM 未启用、失败、无候选或候选全部无效，则回退到原有规则抽取。
7. 规则抽取依次处理提醒偏好、工作偏好。
8. 如果启用 LLM 用户画像抽取，再生成 profile patches，校验后更新 `user_profiles`。
9. 未命中任何规则：只保留 work_events，不生成长期记忆或画像。
...
LONGMEMORY_LLM_API_KEY=<本地环境变量或 .env 提供>
```

默认 provider 为 OpenAI-compatible Volcengine Ark：

```text
LONGMEMORY_LLM_BASE_URL=https://ark.cn-beijing.volces.com/api/v3
LONGMEMORY_LLM_MODEL=doubao-seed-2-0-lite-260215
```

LLM 只允许返回候选 JSON，不允许写库。候选字段固定为：
...
1. `memory_category` 只允许 `WorkPreferenceMemory` 或 `ReminderPreferenceMemory`。
2. `work_type` 只允许当前已有枚举。
3. `confidence` 必须在 0 到 1 之间。
4. `summary`、`preference` 或 `reminder_text` 必须满足对应类别的最小内容要求。
5. `evidence_text` 必须来自当前事件文本。
6. 候选字段包含 token、password、secret、api_key、邮箱、电话、私钥或强脱敏标记时丢弃。
7. `ReminderPreferenceMemory` 的时间结构不信任 LLM；后端只用现有 `ReminderParser` 从 `reminder_text` 或原始事件文本解析。

通过校验后，后端组装 `MemoryWrite` 并调用现有 `MemoryStore.write_memory`，继续复用 `personal_memories`、`memory_evidence_links` 和 `memory_audit_logs`。LLM 未启用、失败、无候选或候选全部无效时，回退到 6.3 和 6.4 的规则抽取。

### 6.6 LLM 用户画像抽取

启用条件和 provider 配置复用 6.5 的 LLM 环境变量。

LLM 用户画像抽取只返回 profile patch，不允许写库。固定 JSON 结构：

```json
{
  "patches": [
    {
      "dimension": "work_identity",
      "claim": "用户当前负责校园比赛相关项目",
      "confidence": 0.82,
      "evidence_text": "我现在在做飞书校园大赛项目"
    }
  ]
}
```
...
| dimension | 说明 |
| --- | --- |
| work_identity | 职位、角色、团队身份、职责范围 |
| current_work_stage | 当前阶段、正在推进的项目、近期目标 |
| work_preferences | 工作方式、输出结构、协作偏好 |
| communication_style | 表达风格、回复长短、语言偏好 |
| tool_usage | 常用工具、文档、表格、知识库倾向 |
| reminder_and_proactive_service | 主动提醒、跟进、周期性服务偏好 |
| life_preferences | 饮食偏好、日程习惯、非敏感生活偏好 |
| other_profile_traits | 其他可用于长期个性化且非敏感的画像信息 |
...
2. `claim` 非空且长度受限。
3. `confidence` 必须在 0 到 1 之间。
4. `evidence_text` 必须来自当前事件文本。
5. 含 token、password、secret、api_key、邮箱、电话、私钥或任何 `[REDACTED_]` 标记的 patch 丢弃。
6. 忘记、取消提醒、删除类语义不更新用户画像。

通过校验后，后端调用 `UserProfileStore.upsert_profile`，更新 `user_profiles.profile_json`，重新渲染 `profile_markdown`，并写入 `user_profile_evidence_links`。同一维度下 claim 规整后重复时不重复追加，只补充证据关系；每个维度只保留有限条高置信 claim，避免画像无限增长。
...
当前数据库由 Alembic 迁移创建，共 7 张业务表。`0001_initial_schema.py` 创建 P0 五张表，`0002_user_profiles.py` 新增用户画像表。
...
| user_profiles | 存储结构化用户画像和 Markdown 派生视图 |
| user_profile_evidence_links | 记录用户画像和证据事件之间的关系 |
...
## 18. user_profiles 和 user_profile_evidence_links 表

`user_profiles` 用途：保存每个用户当前最新的结构化画像快照。

| 字段 | 说明 |
| --- | --- |
| user_id | 用户 ID，主键 |
| tenant_id | 租户 ID，可为空 |
| profile_json | 结构化画像，按 dimension 保存 claims |
| profile_markdown | 由 profile_json 派生的短 Markdown |
| confidence | 画像整体置信度 |
| version | 画像版本，从 1 开始递增 |
| last_event_id | 最近一次更新画像的 work_events.event_id |
| created_at | 创建时间 |
| updated_at | 更新时间 |

`user_profile_evidence_links` 用途：记录画像和证据事件之间的关系。

| 字段 | 说明 |
| --- | --- |
| user_id | 用户 ID |
| event_id | 证据事件 ID |
| relation_type | updated_from 或 reinforced_by |
| created_at | 创建时间 |

插入同样使用 `OR IGNORE`，避免重复证据关系。V1.1 暂不新增 `user_profile_audit_logs`。
...
| 用户画像抽取 | `src/feishu_campus_longmemory/profile/extractor.py` |
| 用户画像存储 | `src/feishu_campus_longmemory/profile/store.py` |
| 用户画像 Markdown 渲染 | `src/feishu_campus_longmemory/profile/renderer.py` |
...
| Profile 和 Context API | `src/feishu_campus_longmemory/api/profile.py` |
| Alembic 迁移 | `alembic/versions/0001_initial_schema.py`、`alembic/versions/0002_user_profiles.py` |
