...
当前显式抽取器只从 `WorkEvent.content_json.summary.text` 读取文本。1.0.1 起可选启用 LLM 候选抽取层，但 LLM 只返回候选结构，不直接写库；候选必须通过后端校验后再进入现有 `MemoryStore`。
...
7. 规则抽取依次处理提醒偏好、工作偏好。
8. 未命中任何规则：只保留 work_events，不生成长期记忆。
...
1.0.1 的 LLM 抽取只改造抽取层，不新增数据库表、字段或迁移。
