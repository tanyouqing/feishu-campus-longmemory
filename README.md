...
P0 正式版交付个人显式记忆最小闭环：真实飞书/OpenClaw 事件进入 Evidence Store，显式偏好与提醒被写入 PersonalMemory，OpenClaw 回复前可获得 Memory Context Pack，到期提醒可通过飞书 Bot 真实发送并回写 Evidence。系统仍只接收来自飞书官方事件回调、OpenClaw Hook，或带鉴权的标准事件入口的数据；不提供 mock 飞书或 mock OpenClaw 接口。
...
P0 正式版默认不启用后台发送，避免本地开发误发真实飞书消息。手动触发已到期提醒：
...
P0 测试覆盖本地工程骨架、SQLite 迁移、健康检查、OpenClaw/飞书事件入库、幂等去重、Evidence 查询鉴权、基础脱敏、显式记忆抽取、提醒任务持久化、记忆更新、软删除、冲突替换、结构化检索排序、Context Pack 隐私边界、OpenClaw Hook 的 `message:preprocessed` 注入配置、主动提醒发送状态管理、成功/失败 Evidence 回写、敏感提醒拦截和 P0 验收脚本静态检查。
